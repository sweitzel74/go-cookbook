...
# If we're upgrading an existing Go Server,
# then leave the configuration and such intact.

# If it's a new node, and a SVN backup URL is specified,
# then restore/overwrite any existing configuration.

# Detection isn't based on the <license> element,
# as Go-Community edition has no license.

# We look for at least one <pipelines> element,
# assuming that if you have a backup,
# you have at least one pipeline.

if ::File.exists?('/etc/go/cruise-config.xml') &&
...
elsif node[:go][:backup_retrieval_type] =~ /local/i &&
      ::File.directory?("#{node[:go][:backup_path]}")

  chef_file_cache_path = Chef::Config[:file_cache_path]
  go_restore_directory =
    "#{chef_file_cache_path}/go-config-restore/go-config/current"

  directory "#{go_restore_directory}" do
    mode 0755
    recursive true
    action :create
  end
  ...
  ruby_block 'copy_local_file' do
    block do
      if ::File.exists?("#{node[:go][:backup_path]}/db.zip")
        ::FileUtils.cp "#{node[:go][:backup_path]}/db.zip",
                       "#{go_restore_directory}"
      end
      if ::File.exists?("#{node[:go][:backup_path]}/config-dir.zip")
        ::FileUtils.cp "#{node[:go][:backup_path]}/config-dir.zip",
                       "#{go_restore_directory}"
      end
      if ::File.exists?("#{node[:go][:backup_path]}/config-repo.zip")
        ::FileUtils.cp "#{node[:go][:backup_path]}/config-repo.zip",
                       "#{go_restore_directory}"
      end
    end
  end
...
# Trash and restore the /etc/go configuration.
# Noted side-effect of this approach is that /etc/go is owned by Go, not root.
directory '/etc/go' do
  action :delete
  recursive true
  not_if { skip_backup }
...
execute 'restore_config' do
  command "unzip -q -u -o #{go_restore_directory}/config-dir.zip -d /etc/go"
  user 'go'
  group 'go'
  action :run
  not_if { skip_backup }
...
# Trash and restore the internal Git repo,
# that's the source of the pipeline config.
# Once this completes fire up Go.
...
# Always run the publish/remove autoregister key blocks,
# so that if the attribute gets changed dynamically we still work properly.
# If it's not run every time,
# we could leave the autoregister key in the attributes which would be bad.

if Chef::Config[:solo]
  Chef::Log.warn(
    'Automatic agent registration is not supported on chef-solo. ' \
    'All attributes must be set on the agent node directly.'
    )
end
...
ruby_block 'publish_autoregister_key' do
  block do
    ...
    Chef::Log.warn(
      'Enabling automatic agent registration. ' \
      'Any configured agent will be configured to build without authorization.'
      )
